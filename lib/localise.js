			...
			if (fileMatchesPattern(file, options.pattern)) {
				...
				await Promise.all(options.locales.map(async function(locale) {
					...
					await Promise.all(nsList.map(ns => {
						const resourceFile = options.nsPath.replace('__ns__',ns).replace('__lng__', locale)
						return fs.promises.readFile(resourceFile).then(res => {
							store[locale] = store[locale] || {}
							store[locale][ns] = JSON.parse(res)
							if (f.resourceFiles) {
								for (const key in f.resourceFiles) {
									const extraFile = options.nsPath.replace('__lng__', locale).replace('__ns__.json', f.resourceFiles[key])
									const value = fs.readFileSync(extraFile, 'utf8')
									store[locale][ns][key] = value
								}
							}
					...
					if (options.frontMatterKeys && options.frontMatterKeys.length) {
						options.frontMatterKeys.forEach(k => {
							if (f[k]) {
								f[k] = f.t(f[k])
							}
						})
